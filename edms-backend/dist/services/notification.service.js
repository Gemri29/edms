...
exports.sendExpiryDigest = sendExpiryDigest;
...
function buildDocEntries(employees) {
    const entries = [];
    const now = new Date();
    const docFields = [
...
// ─── Build HTML email ─────────────────────────────────────────────────────────
function buildEmailHtml(entries, recipientName) {
  ...
  `;
    const tableHtml = (rows, label, color) => `
...
async function sendExpiryDigest() {
    const employees = await (0, employee_service_1.getExpiringEmployees)();
